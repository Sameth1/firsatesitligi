-- ============================================================
-- 119 · Sıkı Hedef Ülke Filtresi (Strict Host Country Filter)
-- ============================================================
-- SORUN:
--   match_opportunities fonksiyonunda:
--   ('*' = any(oc.host_countries) or p_host_country = any(oc.host_countries))
--   şartı nedeniyle, kullanıcı spesifik bir ülke (ör. Almanya / DE) seçtiğinde,
--   host_countries dizisinde '*' (küresel/çevrim içi) olan veya hatalı '*' girilmiş
--   bütün kayıtlar (ör. Türkiye Hükümeti bursları, sanal stajlar vb.) listeye sızıyordu.
--
-- ÇÖZÜM:
--   Kullanıcı belirli bir ülke seçtiğinde yalnız o ülkeyi içeren kayıtlar döner:
--   (p_host_country is null or p_host_country = any(oc.host_countries))
--   Ülke seçilmediğinde (null) ise küresel (*) dahil tüm 93 fırsat eksiksiz listelenir.
...
as $function$
  ...
  where opp.is_active = true
    ...
    and (p_study_level is null or p_study_level = 'any' or 'any' = any(oc.study_level) or p_study_level = any(oc.study_level))
